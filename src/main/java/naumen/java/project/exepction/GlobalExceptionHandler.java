...
public class GlobalExceptionHandler {
    ...
    /**
     * Ошибка - формат экспорта не существует (не получается распарсить enum ExportFormat)
     */
    ...
    public ResponseEntity<ExceptionResponseDTO> handleHttpMessageNotReadable(HttpMessageNotReadableException exception,
        ...
        if (cause instanceof InvalidFormatException ife && ife.getTargetType() == ExportFormat.class) {
            String badValue = String.valueOf(ife.getValue());
            String allowed = Arrays.stream(ExportFormat.values())
                    .map(Enum::name)
                    .collect(Collectors.joining(", "));

            ExceptionResponseDTO error = new ExceptionResponseDTO(
                    HttpStatus.BAD_REQUEST,
                    "Формат экспорта '" + badValue + "' не поддерживается. Допустимые значения: " + allowed,
                    request.getRequestURI(),
